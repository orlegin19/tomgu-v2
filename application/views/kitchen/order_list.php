...
window.load_data = function($id=''){
    if($id == '')
        ...
        $.ajax({
        url:'<?php echo base_url().'kitchen/load_orders' ?>',
        method:'POST',
        data:{id:$id},
        error:err=>{
            console.log(err)
                ...
                if(typeof data != undefined){
                    if(Object.keys(data).length > 0){
                        ...
                        Object.keys(data).map(k=>{
                            ...
                            card.find('.card-title').html(data[k].ref_id)
                            card.find('.card-text').html(data[k].queue)
                            card.attr({'data-id':data[k].id,'data-queue':data[k].queue,'data-ref':data[k].ref_id})
                            if(data[k].serve_status == 1)
                            card.find('.card-status').html('<div class="badge badge-success">Partially Served</div>')
                            if(data[k].serve_status == 2)
                            card.find('.card-status').html('<div class="badge badge-success">Served</div>')
